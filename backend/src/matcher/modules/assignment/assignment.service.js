...
class AssignmentService {
    ...
    createAssignment(requestId, decision, ambulance) {
        /*
         * Prevent duplicate active assignments.
         */
        const existingActiveAssignment = this.getActiveAssignment(requestId);
        if (existingActiveAssignment) {
            throw new Error(`Active assignment ${existingActiveAssignment.assignmentId} already exists for request ${requestId}`);
        }
        /*
        ...
        if (ambulance.availabilityStatus !==
            "AVAILABLE") {
            throw new Error(`Ambulance ${ambulance.ambulanceId} is not available`);
        }
        const assignment = {
         ...
         * for the emergency.
         */
        this.activeAssignments.set(requestId, assignment.assignmentId);
        /*
         * Reserve ambulance.
         */
    ...
    updateStatus(assignmentId, status, rejectionReason) {
        ...
        if (assignment.status === "REJECTED" ||
            ...
            throw new Error(`Assignment ${assignmentId} has already been closed with status ${assignment.status}`);
        }
        ...
        if (status === "REJECTED" ||
            status === "TIMEOUT" ||
            status === "CANCELLED") {
            const activeAssignmentId = this.activeAssignments.get(assignment.requestId);
            if (activeAssignmentId ===
                assignment.assignmentId) {
                this.activeAssignments.delete(assignment.requestId);
            }
        }
        return assignment;
    }
     ...
     */
    respondToActiveAssignment(requestId, ambulanceId, status, rejectionReason, ambulances) {
        const assignment = this.getActiveAssignment(requestId);
        if (!assignment) {
            throw new Error(`No active assignment found for request ${requestId}`);
        }
        if (assignment.ambulanceId !==
            ambulanceId) {
            throw new Error(`Ambulance ${ambulanceId} is not the active responder for request ${requestId}`);
        }
        /*
         * Update assignment first.
         */
        const updatedAssignment = this.updateStatus(assignment.assignmentId, status, rejectionReason);
        /*
         ...
         */
        if (ambulances) {
            const ambulance = ambulances.find((item) => item.ambulanceId ===
                ambulanceId);
            if (ambulance) {
                ambulance.availabilityStatus =
                    "AVAILABLE";
    ...
    getActiveAssignment(requestId) {
        const assignmentId = this.activeAssignments.get(requestId);
        if (!assignmentId) {
            return undefined;
        }
    ...
    clearActiveAssignment(requestId) {
        this.activeAssignments.delete(requestId);
    }
}
