...
class DispatchEngineService {
    constructor(candidateFilterService, etaService, scoringService) {
    ...
    dispatch(request, ambulances, searchRadiusKm, excludedAmbulanceIds = new Set()) {
        ...
        const maxTravelTime = Math.max(...candidateData.map((candidate) => candidate.eta.estimatedMinutes));
        const maxDistance = Math.max(...candidateData.map((candidate) => candidate.eta.route.distanceKm));
        ...
        const maxTrafficCost = Math.max(...trafficCosts);
        ...
        const scoredCandidates = candidateData.map((candidate, index) => {
            const scoreInput = {
                travelTimeMinutes: candidate.eta.estimatedMinutes,
